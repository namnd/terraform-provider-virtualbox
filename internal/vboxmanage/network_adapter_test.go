...
func TestFormatMACAddress(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "uppercase without separators", mac: "080027EEA5E7", want: "08:00:27:EE:A5:E7"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := FormatMACAddress(tt.mac); got != tt.want {
				t.Fatalf("FormatMACAddress(%q) = %q, want %q", tt.mac, got, tt.want)
			}
		})
	}
}
